...
public abstract class DynamicJdbcEntityDao<T, ID extends Number> extends
		JdbcDaoSupport {
	...
	private BeanInfo getBeanInfo() {
		try {
			...
			throw new IllegalArgumentException(
					"error: generate Table instance from Class,clazz:"
							+ entityClass, e);
		}
	}
	...
	private String getColumnSqlName(PropertyDescriptor pd, Method readMethod) {
		String sqlName = null;
	...
	public int[] update(T[] entities) {
		...
		return new NamedParameterJdbcTemplate(this.getDataSource())
				.batchUpdate(getUpdateByPkSql(), batch);
	}
	...
	protected String setOrderParameterToSql(final String sql,
		...
		for (Sort orderBy : pageRequest.getSort()) {
			builder.append(String.format(" %s %s,", TransCodingUtils
					.camel4underline(orderBy.getProperty()), orderBy.getDir()));
		}
	...
	public String getTableName() {
		if (tableName == null) {
			throw new IllegalArgumentException("set tableName first");
		}

...
public abstract class JdbcEntityDao<T, ID extends Number> extends JdbcDaoSupport{
	...
	public int[] update(T[] entities) {
		...
		//JdbcTemplate 不支持 SqlParameterSource的批量更新 
	...
	public Page<T> findPage(final PageRequest pageRequest, String sql,
		...
		page.setAaData(result);
		return page;
	}

	
	
	public Page<T> findPage1(final PageRequest pageRequest, String sql,
			final Object... values) {
		Validate.notNull(pageRequest, "page不能为空");
		...
		if (pageRequest.isCountTotal()) {
			 List<Map<String,Object>> countSqlResult1 = countSqlResult1(sql, values);
			page.setITotalRecords(countSqlResult1.size());
			page.setITotalDisplayRecords(countSqlResult1.size());
		}
	...
	protected String setPageParameterToQuery(String sql,
		...
		sb.append(", ");
		sb.append(pageRequest.getIDisplayLength());
		return sb.toString();
	}
	...
	protected List<Map<String, Object>> countSqlResult1(final String sql, final Object... values) {
		String countSql = prepareCountSql(sql);

		try {
			List<Map<String, Object>> queryForList = this.getJdbcTemplate().queryForList(countSql,values);
			return queryForList;
		} catch (Exception e) {
			throw new RuntimeException("hql can't be auto count, hql is:"
					+ countSql, e);
